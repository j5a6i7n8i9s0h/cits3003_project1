...
void loadMeshIfNotAlreadyLoaded(int meshNumber)
{
    if (meshNumber>=numMeshes || meshNumber < 0) {
        printf("Error - no such model number: %d", meshNumber);
        exit(1);
    }
    ...
    aiScene* scene = loadScene(meshNumber);
    ...
    CheckError();
     ...
     GLuint bonebuffers[2];
     glGenBuffers( 2, bonebuffers );  // Add two vertex buffer objects

     glBindBuffer( GL_ARRAY_BUFFER, bonebuffers[0] ); CheckError();
     ...
     cout << "made it to the end of load" << endl;
}
...
void init( void )
{
    ...
    colourEffectU = glGetUniformLocation(shaderProgram, "ColourEffect");
    timeU = glGetUniformLocation(shaderProgram, "time"); CheckError();
    randomU = glGetUniformLocation(shaderProgram, "randoms");

    //part2 adding bone shader variables
    ...
    //cout << "Attributes" << vPosition << " " << vNormal << " " << vTexCoord << " " << vBoneWeights << " " << vBoneIDs <<endl;
    //cout << "Uniforms" << projectionU << " " << modelViewU << " " <<rippleEffectU << " " << timeU << " " <<uBoneTransforms;
    ...
    addObject(55);
    sceneObjs[2].loc = vec4(-2.0,1.0,1.0,0);
    sceneObjs[2].scale = 0.2;
    sceneObjs[2].texId = 0;
    sceneObjs[2].brightness = 0.5;

    addObject(59); // A test mesh
    sceneObjs[3].scale = 0.06;
...
void drawMesh(SceneObject sceneObj)
{
    ...
    glUniform1f(timeU, floor(timer*0.05));
    ...
    glUniform1i(colourEffectU, colourEffect);
    double a = (double)rand() / rand();
    double b = (double)rand() / rand();
    double c = (double)rand() / rand();
    glUniform3f(randomU, fmod(a,1),fmod(b,1),fmod(c,1));
    mat4 model = Translate(sceneObj.loc) * Scale(sceneObj.scale);
    model = model *RotateX(sceneObj.angles[0])*RotateY(sceneObj.angles[1])*RotateZ(sceneObj.angles[2]);
    ...
    calculateAnimPose(meshes[sceneObj.meshId], scenes[sceneObj.meshId], 0,
                      fmod(timer*0.01,120.0), boneTransforms);
...
void display( void )
{
    ...
    for (int i=0; i < nObjects; i++) {
        ...
        glUniform3fv( glGetUniformLocation(shaderProgram, "SpecularProduct1"), 1, so.specular * so.rgb * so.brightness * 2.0);
        CheckError();
        ...
        glUniform3fv( glGetUniformLocation(shaderProgram, "SpecularProduct2"), 1, so.specular *  so.rgb * so.brightness * 2.0);
        CheckError();
...
static void SaveScene(int id)
{
  ...
  if(slotfile!=NULL)
  {
    ...
    fwrite(&colourEffect,sizeof(int),1,slotfile);
 ...
 static void LoadScene(int id)
{
  ...
  {
    // fwrite (void* of data, size*t , int, FILE* ) general layyout
     fread(&colourEffect,sizeof(int),1,slotfile);
...
static void makeMenu()
{
    ...
    int colourMenuId = glutCreateMenu(colourMenu);
    glutAddMenuEntry("Normal", 0);
    glutAddMenuEntry("Random Colours", 90);
    ...
    glutAddSubMenu("ColourEffects", colourMenuId);
